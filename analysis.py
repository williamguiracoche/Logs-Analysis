...
def popular_articles():
  ...
  cursor = db.cursor()
  cursor.execute("select slug, count(*) as num from log, articles where status = '200 OK' and not path = '/' and log.path like '%' || articles.slug group by path, slug order by num desc limit 3;")
